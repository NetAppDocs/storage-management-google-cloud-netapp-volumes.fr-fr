...
NOTE: Vous pouvez utiliser la console Google Cloud ou l'interface de ligne de commande gcloud pour ces tâches. Pour utiliser l'interface de ligne de commande gcloud, vous devez d'abord link:https://cloud.google.com/sdk/docs/install-sdk["installé l'interface de ligne de commande Google Cloud"^].
...
NOTE: Les modifications apportées aux autorisations des projets Google Cloud peuvent prendre quelques minutes avant d'être effectives.
...
NOTE: Pour les environnements de test, utilisez  `credentials-sa@wf-staging-netapp.iam.gserviceaccount.com` au lieu du compte de service de production. Remplacez la valeur du paramètre  `--member` dans la commande ci-dessus par  `serviceAccount:credentials-sa@wf-staging-netapp.iam.gserviceaccount.com`.
